...
const GastosAdminRow = ({ data }: GastosAdminRowProps) => {
  ...
  const periodoAplicaFormateado = data.periodo_aplica
    ? new Date(data.periodo_aplica).toLocaleDateString('es-AR', {
        month: 'long',
        year: 'numeric',
        timeZone: 'UTC'
      })
      ...
      <Grid container spacing={2}>
        <Grid item xs={3} sm={1.5}>
          <Stack spacing={0.5}>
            ...
            <Chip
              color={data.tipo_gasto === 'ordinario' ? 'primary' : 'warning'}
              label={data.tipo_gasto}
              size="small"
              variant="light"
              sx={{ maxWidth: 120 }}
            />
        ...
        <Grid item xs={3} sm={1.5}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="body2">{data.comprobante || '-'}</Typography>
          </Stack>
        </Grid>
        ...
        <Grid item xs={3} sm={1.5}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="body2">{periodoAplicaFormateado}</Typography>
          </Stack>
        </Grid>
        <Grid item xs={3} sm={1.5}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="body2">{data.Rubro?.rubro || '-'}</Typography>
          </Stack>
        </Grid>
        <Grid item xs={3} sm={1.5} sx={{ textAlign: 'right' }}>
          <Typography variant="caption" color="textSecondary">
            Monto
          </Typography>
